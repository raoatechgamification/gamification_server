  ...
  try {
    ...
    if (decoded.role === "user") {
      ...
      if (user.isDisabled) {
        return res
          .status(403)
          .json({ status: false, message: "User account is disabled" });
      }

      if (user.isArchived) {
        return res
          .status(403)
          .json({ status: false, message: "User account is archived" });
      }
    }

    // Check if user is a subAdmin
    if (decoded.role === "subAdmin") {
      const subAdmin = await SubAdmin.findById(decoded.id).populate(
        "permissions"
      );
      if (!subAdmin) {
      ...
      if (!subAdmin.roles || subAdmin.roles.length === 0) {
        return res
          .status(401)
          .json({ message: "SubAdmin has no assigned roles" });
      }
...
export const authorize = (
  roles: ("user" | "admin" | "subAdmin" | "superAdmin")[]
) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (userRole === "subAdmin") {
      ...
      if (
        !role ||
        !role.permissions ||
        role.permissions.some((perm) => typeof perm === "string")
      ) {
        return res
          .status(403)
          .json({ message: "Invalid or missing permissions for subadmin" });
      }
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!req.subadminPermissions || req.subadminPermissions.length === 0) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "No permissions assigned to subadmin",
        });
    }
